...
def run(db):
    ...
    if "products" not in db.list_collection_names():
        console.print("\nℹ Creando colección 'products' con datos de ejemplo...")
    ...
    while True:
        ...
        if choice == "0":
            ...
            if not indexes:
                console.print("ℹ No hay índices definidos (solo el índice _id por defecto)")
            else:
            ...
            if len(indexes) <= 1:
                console.print("ℹ No hay índices adicionales para eliminar")
            else:
            ...
            if not any(field in str(idx) for idx in collection.index_information().values()):
                console.print(f"\nℹ Creando índice temporal en '{field}' para la prueba...")
